...
# strict_ut_027 shared dual-side UT for _shift_input_embeds_kernel.
...
# re-exported by gpu/test_shift_input_embeds_kernel.py and
# npu/test_shift_input_embeds_kernel.py. The side runtime (CUDA or Ascend
# NPU) is injected through the ``rt`` pytest fixture, which provides
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (16, 512, "random", 16, 64),
        (32, 1024, "random", 16, 128),      # large batch
        (64, 256, "random", 16, 64),        # max batch
        (16, 2048, "random", 16, 256),
        (8, 5120, "random", 16, 128),       # production hidden (Llama-class)
        (4, 7168, "random", 32, 256),       # large hidden (Kimi/Qwen-class)
        (16, 512, "long_query", 16, 64),    # long queries
        (8, 2048, "long_query", 32, 256),   # long queries + large hidden
    ],
    ids=lambda v: str(v),
)
def test_shift_input_embeds(
    ...
    if scenario == "long_query":
        ...
        # per-req lens up to max(16, bq + 1)
        num_tokens = max(256, num_reqs * (max(16, bq + 2)))
