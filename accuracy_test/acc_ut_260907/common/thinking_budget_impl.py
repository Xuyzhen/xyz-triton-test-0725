...
# strict_ut_027 shared dual-side UT for _thinking_budget_kernel.
...
def _nan_row(num_tokens: int, vocab_size: int, device) -> torch.Tensor:
...
SHAPE_PARAMS = [
    ...
    # Longer marker sequences (multi-token reasoning markers at production
    # scale; DeepSeek-R1 style <think>/<think> markers are multi-token).
    (2, 1, 3),
    (4, 4, 5),
    (1, 3, 4),
    (5, 2, 6),
...
def test_thinking_budget(
    ...
    if not torch.equal(dev_logits, ref_logits):
        ...
        if len(first_idx) > 0:
            ...
            dev_val = float(dvs_np[loc]) if not _np.isnan(dvs_np[loc]) else float("nan")
            ref_val = float(ref_np[loc]) if not _np.isnan(ref_np[loc]) else float("nan")
            first_info = (
                f"; first mismatch at {loc}: device={dev_val} ref={ref_val}"
            )
        pytest.fail(
...
def test_import_error() -> None:
    if _import_error is not None:
        pytest.fail(
            f"Failed to import _thinking_budget_kernel:\n"
            f"{_import_traceback}"
        )
