...
# strict_ut_027 shared dual-side UT for _update_committed_marker_cache_kernel.
...
SHAPE_PARAMS = [
    ...
    # Larger marker lengths (multi-token markers at production scale).
    (4, 4, 4, 8),
    (4, 4, 4, 1024),
    (8, 4, 8, 16),
    # Larger blocks (fewer, bigger cold-scan chunks).
    (2, 2, 2, 256),
    (3, 2, 3, 2048),
    (1, 1, 1, 4096),
    # Larger max_len (drives scan_pos clamping further from 0).
    (2, 2, 64, 8),
    (3, 3, 256, 16),
]
...
def test_update_committed_marker_cache(
    ...
    # Device run: clone mutated tensors.
    dev_inputs = {
...
def test_import_error() -> None:
    if _import_error is not None:
        pytest.fail(
            f"Failed to import _update_committed_marker_cache_kernel:\n"
            f"{_import_traceback}"
        )
