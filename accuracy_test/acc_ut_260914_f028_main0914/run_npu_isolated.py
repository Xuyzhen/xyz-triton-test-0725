...
"""Run every acc_ut_260914_f028_main0914 NPU-side test module in a fresh process.

Reuses the isolation model of acc_ut_260907 / strict_ut_027 / easy_ut_026: an
Ascend vector-core exception in one kernel poisons the current process device
...
# acc_ut_260914_f028_main0914 directory; the repo root (two levels up) is
# where pytest must run so that
# ``from accuracy_test.acc_ut_260914_f028_main0914...`` resolves.
...
def main() -> int:
    ...
    if not extra_args:
        ...
        # Forward extra args to a single fresh pytest invocation (they may
        # be a filename, a -k filter, or arbitrary pytest flags).
        selected = ()
    ...
    if failures:
        print("\nNPU acc_ut_260914_f028_main0914 modules with failures:")
        for name, returncode in failures:
            print(f"  {name}: pytest exit {returncode}")
        return 1

    print(
        f"\nAll {len(files)} NPU acc_ut_260914_f028_main0914 modules passed "
