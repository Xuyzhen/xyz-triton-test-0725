# GENERATED STRICT UT. Source: accuracy_test/codex/existing_accuracy_tests/from_vllm_ascend/test_temperature.py
...
# Accuracy UT source: vllm-ascend-xyz/tests/e2e/nightly/single_node/ops/singlecard_ops/triton/test_temperature.py
# Kernel source: vllm-ascend-xyz/vllm_ascend/worker/v2/sample/gumbel.py
# Coverage: _temperature_kernel via apply_temperature
...
try:
    ...
    pytest.skip(
        f"installed vLLM does not provide apply_temperature; precision was not tested: {exc}",
...
def test_temperature_kernel(num_tokens, vocab_size):
    ...
    # Build input tensors
    logits_triton = torch.randn((num_tokens, vocab_size), dtype=torch.float32).cuda()
    logits_ref = logits_triton.clone()

    num_requests = num_tokens
    expanded_idx_mapping = torch.arange(num_tokens, dtype=torch.int32).cuda()

    # Include edge cases: 0.0 and 1.0 should leave logits unchanged
    temperature = torch.rand(num_requests, dtype=torch.float32).cuda()
