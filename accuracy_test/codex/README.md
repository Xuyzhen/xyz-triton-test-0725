...
| `test_prepare_dflash_inputs_kernel.py` / `_prepare_dflash_inputs_kernel` | Reimplemented as `_prepare_dflash_inputs_kernel_ascend` and monkey-patched back to the vLLM name; older installed packages may not contain worker-v2 DFlash | `vllm_ascend/worker/v2/spec_decode/dflash/speculator.py:153`; `vllm_ascend/patch/worker/patch_v2/patch_triton.py:37` | `test_prepare_dflash_inputs_kernel_patch.py` | Direct Ascend implementation when available; module-level SKIP otherwise |
