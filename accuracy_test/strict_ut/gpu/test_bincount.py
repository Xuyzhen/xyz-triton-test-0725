# GENERATED STRICT UT. Source: accuracy_test/codex/existing_accuracy_tests/from_vllm_ascend/test_bincount.py
...
# Accuracy UT source: vllm-ascend-xyz/tests/e2e/nightly/single_node/ops/singlecard_ops/triton/test_bincount.py
# Kernel source: vllm-ascend-xyz/vllm_ascend/worker/v2/sample/penalties.py
# Coverage: _bincount_kernel (direct; source test is skipped)
...
def test_bincount_kernel():
    ...
    expanded_idx_mapping = torch.tensor([63], dtype=torch.int32).cuda()
    ...
    prompt_bin_mask = torch.zeros(size=(64, 4748), dtype=torch.int32).cuda()
    output_bin_counts = torch.zeros(size=(64, 151936), dtype=torch.int32).cuda()

    ref_prompt_bin_mask = torch.zeros(size=(64, 4748), dtype=torch.int32).cuda()
    ref_output_bin_counts = torch.zeros(size=(64, 151936), dtype=torch.int32).cuda()
