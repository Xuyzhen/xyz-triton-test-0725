# GENERATED STRICT UT. Source: accuracy_test/codex/existing_accuracy_tests/from_vllm_ascend/test_compute_slot_mapping.py
...
# Accuracy UT source: vllm-ascend-xyz/tests/e2e/nightly/single_node/ops/singlecard_ops/triton/test_compute_slot_mapping.py
...
def test_compute_slot_mapping_npu_kernel():
    ...
    device = "cuda" if torch.cuda.is_available() else "cuda" if torch.cuda.is_available() else "cpu"
    ...
    block_table_ptrs = torch.tensor([block_tables[0].data_ptr()], dtype=torch.uint64, device=device)
