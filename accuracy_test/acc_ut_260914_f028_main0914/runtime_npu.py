...
"""Self-contained NPU runtime helpers for acc_ut_260914_f028_main0914 tests.

Derived from acc_ut_260907/runtime_npu.py (which itself derives from
strict_ut_027 / easy_ut_026 and has no dependency on any sibling
``accuracy_test.*`` package, so the suite can be dropped anywhere and still
collect) and renamed for the acc_ut_260914_f028_main0914 project.
...
if not hasattr(torch, "npu") or not torch.npu.is_available():
    pytest.skip(
        "Ascend NPU is required for acc_ut_260914_f028_main0914 NPU-side "
        "accuracy tests (run the GPU side via run_gpu.sh on CUDA hosts)",
...
def _install_vllm_triton_utils_shim() -> types.ModuleType:
    ...
    The installed Triton may predate ``triton.experimental.gluon``.
    ...
    can break collection on such hosts.

    Only that one ``__init__`` line needs bypassing. So instead of replacing
    ...
    installed triton. Submodules that do ``from vllm.triton_utils import
    ...
    if "vllm.triton_utils" in sys.modules:
        ...
        if getattr(existing, "_acc_ut_260914_shim", False):
            return existing
    ...
    def _placeholder_lazy(name: str):
        def _fn(*args, **kwargs):  # pragma: no cover
            ...
            raise RuntimeError(
                ...
                "unavailable under the precision-UT shim for this Triton"
            )
...
def ensure_default_ascend_config() -> None:
    ...
    ``vllm_ascend.sample.sampler`` helpers read ``get_ascend_config()``
    attributes on every call, but a UT process never starts the engine, so
    the singleton is normally uninitialized and ``get_ascend_config()``
    would raise. Install a minimal default unless a real config is already
    present. Idempotent.
    """
    try:
