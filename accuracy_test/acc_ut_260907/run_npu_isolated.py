...
"""Run every acc_ut_260907 NPU-side test module in a fresh process.

Reuses the isolation model of strict_ut_027 / easy_ut_026: an Ascend
vector-core exception in one kernel poisons the current process device
...
# acc_ut_260907 directory; the repo root (two levels up) is where pytest
# must run so that ``from accuracy_test.acc_ut_260907...`` resolves.
...
def main() -> int:
    ...
    if failures:
        print("\nNPU acc_ut_260907 modules with failures:")
        for name, returncode in failures:
            print(f"  {name}: pytest exit {returncode}")
        return 1

    print(f"\nAll {len(files)} NPU acc_ut_260907 modules passed or explicitly skipped.")
    return 0
