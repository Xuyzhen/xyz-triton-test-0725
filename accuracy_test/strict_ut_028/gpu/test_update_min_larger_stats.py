...
# GPU counterpart of npu/test_update_min_larger_stats.py: identical shapes,
# dtypes, seeds and data generation for GPU-NPU comparison (double-benchmark
# per Precision Standard 2.1 sec 4.5: CPU reference is the Golden, GPU and NPU
# are both evaluated against it).
...
from accuracy_test.strict_ut.runtime_gpu import DEVICE, init_device_properties_triton, synchronize
...
def test_no_above_entries():
    ...
    torch.testing.assert_close(
        min_larger.cpu(), min_larger_expected.cpu(), rtol=0, atol=0, check_dtype=True
    )
