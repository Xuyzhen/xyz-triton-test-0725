...
# vLLM vanilla kernel: _compute_max_and_sumexp / _compute_block_stats_kernel from
...
The _compute_max_and_sumexp helper is called from _compute_block_stats_kernel.
        ...
        for li in range(num_logits):
            ...
            for bi in range(vocab_num_blocks):
                ...
                if temp == 0.0:
                    ...
                    # NOTE: The kernel computes draft max/sumexp on the raw
                    # draft_logits WITHOUT temperature scaling. Temperature is
                    # only applied later during rejection sampling. Matching
                    # kernel behavior, the reference must NOT divide by temp.
