#!/usr/bin/env bash
#
# 一键运行 acc_ut_260914_f028_main0914 测试集：自动检测当前机器可用设备并运行对应侧。
#
...
if command -v python >/dev/null 2>&1; then
  ...
  echo "[ERROR] 未找到 python / python3，请先激活对应环境" >&2
  exit 1
fi
...
"${PY}" - <<'PY' && HAS_CUDA=1 || true
import torch
raise SystemExit(0 if torch.cuda.is_available() else 1)
PY

"${PY}" - <<'PY' && HAS_NPU=1 || true
import torch
...
if [[ "$RAN_ANY" == "0" ]]; then
  echo "[ERROR] 当前机器既无 CUDA GPU 也无昇腾 NPU，无法运行 acc_ut_260914_f028_main0914" >&2
  exit 1
fi
