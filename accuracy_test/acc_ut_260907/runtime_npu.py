...
"""Self-contained NPU runtime helpers for acc_ut_260907 accuracy tests.

Copied from strict_ut_027/runtime_npu.py (which itself derives from
easy_ut_026 and has no dependency on any sibling ``accuracy_test.*``
package, so the suite can be dropped anywhere and still collect) and
renamed for the acc_ut_260907 consolidated project.
...
if not hasattr(torch, "npu") or not torch.npu.is_available():
    pytest.skip(
        "Ascend NPU is required for acc_ut_260907 NPU-side accuracy tests "
...
def _install_vllm_triton_utils_shim() -> types.ModuleType:
    ...
    if "vllm.triton_utils" in sys.modules:
        ...
        if getattr(existing, "_acc_ut_260907_shim", False):
            return existing
