# GENERATED STRICT UT. Source: accuracy_test/codex/existing_accuracy_tests/from_vllm/test_rejection_kernel.py
...
class TestRejectionKernel:
    ...
    def _run_non_greedy_kernel(self, target_logits, draft_logits, draft_sampled,
                               num_reqs, num_draft_tokens, vocab_size, temperature):
        """Launch stats + rejection kernels for a deterministic non-greedy case.

        Mirrors the NPU strict UT scenarios: the philox draw cannot be
        reproduced on CPU, so callers construct targets whose acceptance
        margin is sign-definite for every u in (0, 1).
        """
        max_num_reqs = 1
        num_logits = num_reqs * (num_draft_tokens + 1)
        num_speculative_steps = num_draft_tokens
        ...
        torch.cuda.synchronize()
        return sampled.cpu(), rejected_steps.cpu()
    ...
    def test_non_greedy_always_accept(self):
        ...
        draw u in (0, 1), so acceptance is deterministic (all accepted).
        Mirrors the NPU strict UT scenario.
        """
        num_reqs = 1
        num_draft_tokens = 2
        vocab_size = 128
        ...
        target_logits = torch.randn(num_logits, vocab_size, dtype=torch.float32, device=self.device)
        draft_sampled = torch.randint(0, vocab_size, (num_logits + 1,), dtype=torch.int64, device=self.device)
        # Near-one-hot target at each draft token: fp32 sumexp of the rest
        # underflows, so target_log_prob == 0 exactly.
        for i in range(num_logits - 1):
            target_logits[i, draft_sampled[i + 1]] = 100.0
        draft_logits = torch.randn(1, num_draft_tokens, vocab_size, dtype=torch.float32, device=self.device)
        temperature = torch.full((1,), 1.0, dtype=torch.float32, device=self.device)

        _, rejected_steps = self._run_non_greedy_kernel(
            target_logits, draft_logits, draft_sampled,
            num_reqs, num_draft_tokens, vocab_size, temperature,
        )

        assert rejected_steps[0].item() == num_draft_tokens, (
            f"Expected {num_draft_tokens} accepted, got {rejected_steps[0].item()}"
        )

    def test_non_greedy_always_reject(self):
        """Non-greedy with draft logits: draft token impossible under target.

        target_log_prob - draft_log_prob is far below log(u_min), so
        acceptance is deterministic (nothing accepted). Mirrors the NPU
        strict UT scenario.
        """
        num_reqs = 1
        num_draft_tokens = 2
