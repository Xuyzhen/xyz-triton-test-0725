...
# easy_ut_026 strict UT for _prepare_input_buffers_kernel.
...
def _run_kernel(inputs: dict[str, Any], device: str) -> dict[str, torch.Tensor]:
    """Run the kernel on the given device and return the output tensors."""
...
def _assert_bitwise(name: str, expected: torch.Tensor, actual: torch.Tensor,
                    device: str) -> None:
    """Assert bitwise equality for integer tensors."""
...
def test_prepare_input_buffers(
    ...
    """Bitwise comparison of NPU kernel output vs independent CPU reference."""
