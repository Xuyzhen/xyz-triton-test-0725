# GENERATED STRICT UT. Source: accuracy_test/codex/missing_accuracy_tests/test_probabilistic_rejection_kernel_patch.py
...
# vllm-ascend/vllm_ascend/worker/v2/spec_decode/rejection_sampler_utils.py:192
...
- Draws u ~ U(0,1) via philox: u_seed = tl.randint(seed, int32(pos)), then
  ...
  and the Ascend vector core has no uint64 umulhi, so the 32-bit philox path
  with a 1-element block is used, same pattern as _npu_gumbel_block_argmax)
...
- Rejection uses the probability ratio test p(x) > u * q(x), i.e.
  target_log_prob > tl.log(u) + draft_log_prob (a real stochastic check)
...
The exact philox stream cannot be reproduced on CPU, so the non-greedy
tests below use deterministic scenarios instead of seeded draws:
- always-accept: target near-one-hot at the draft token, so
  target_log_prob >= 0 >= log(u) + draft_log_prob for every u < 1
- always-reject: the draft token has ~-100 logit under the target, so
  target_log_prob - draft_log_prob < log(2^-31) fails for every u >= 2^-31
"""

import torch

from vllm.triton_utils import tl, triton
from accuracy_test.acc_ut_260907.runtime_npu import init_device_properties_triton
...
def _non_greedy_accept_cpu_ref(
    ...
    draft_local_max: torch.Tensor | None,
    draft_local_sumexp: torch.Tensor | None,
    start_idx: int,
    num_tokens: int,
    req_state_idx: int,
    vocab_num_blocks: int,
    ...
    """CPU reference for the non-greedy probabilistic rejection test.

    The kernel draws u ~ U(0,1) (philox, clamped to [2^-31, 1)) and accepts
    iff target_log_prob > log(u) + draft_log_prob. The exact philox stream
    cannot be reproduced on CPU, so this reference evaluates the two
    deterministic extremes of the u range instead:
      - target_log_prob - draft_log_prob >= 0      -> accepted for every u < 1
      - target_log_prob - draft_log_prob < log(2^-31) -> rejected for every u
    Returns (accepted_count, "always" | "never" | "stochastic").
    """
    import math
    ...
    for i in range(num_tokens - 1):
        ...
        if has_draft_logits and draft_logits is not None:
            dl_max_vals = draft_local_max[logit_idx, :vocab_num_blocks]
            dl_sumexp_vals = draft_local_sumexp[logit_idx, :vocab_num_blocks]
            draft_lse_val = float(dl_max_vals.max()) + math.log(
                float(dl_sumexp_vals.sum())
            )
            draft_logit = draft_logits[req_state_idx, i, draft_token].item()
            draft_log_prob = draft_logit - draft_lse_val
        else:
            # One-hot draft: q(draft_token) = 1 -> log_q = 0.
            draft_log_prob = 0.0

        margin = target_log_prob - draft_log_prob
        if margin >= 0.0:
            # p/q >= 1 > u for every u < 1: deterministic accept.
            verdicts.append("always")
            accepted_count += 1
        elif margin < math.log(2.0**-31):
            # p/q < 2^-31 <= u for every u: deterministic reject.
            verdicts.append("never")
        else:
            # Acceptance depends on the philox draw; deterministic tests
            # must not land in this band.
            verdicts.append("stochastic")

    overall = (
        "always" if all(v == "always" for v in verdicts)
        else "never" if all(v == "never" for v in verdicts)
        else "stochastic"
    )
    return accepted_count, overall, target_lse, draft_lse_val
...
class TestProbabilisticRejectionKernelPatch:
    ...
    def test_non_greedy_always_accept(self):
        """Non-greedy with draft logits: target near-one-hot at the draft token.

        target_log_prob ~= 0 >= log(u) + draft_log_prob for every philox
        draw u < 1, so acceptance is deterministic (all steps accepted).
        """
        num_reqs = 1
        num_spec_steps = 2
        num_logits = num_spec_steps + 1
        ...
        target_logits = torch.randn(num_logits, vocab_size, dtype=torch.float32, device=self.device)
        draft_sampled = torch.randint(0, vocab_size, (num_logits,), dtype=torch.int32, device=self.device)
        # Make the target near-one-hot at each draft token: the remaining
        # logits underflow in fp32 sumexp, so target_log_prob == 0 exactly.
        for i in range(num_logits - 1):
            target_logits[i, draft_sampled[i + 1]] = 100.0
    ...
    def test_non_greedy_always_reject(self):
        """Non-greedy with draft logits: draft token impossible under target.

        target_log_prob - draft_log_prob < log(2^-31) <= log(u) for every
        philox draw, so acceptance is deterministic (no steps accepted).
        """
        num_reqs = 1
        num_spec_steps = 2
        num_logits = num_spec_steps + 1
        ...
        target_logits = torch.randn(num_logits, vocab_size, dtype=torch.float32, device=self.device)
        draft_sampled = torch.randint(0, vocab_size, (num_logits,), dtype=torch.int32, device=self.device)
        # Make the draft token impossible under the target: its log-prob
        # margin drops far below the u clamp floor 2^-31.
        for i in range(num_logits - 1):
            target_logits[i, draft_sampled[i + 1]] = -100.0
        temperature = torch.tensor([1.0], dtype=torch.float32, device=self.device)
        seeds = torch.tensor([42], dtype=torch.int64, device=self.device)
    ...
    def test_non_greedy_no_draft_logits(self):
        """Non-greedy without draft logits (one-hot): near-one-hot target.

        draft_log_prob = 0 and target_log_prob ~= 0 > log(u) for every
        philox draw u < 1, so acceptance is deterministic.
        """
        num_reqs = 1
        num_spec_steps = 3
        num_logits = num_spec_steps + 1
    ...
    def test_non_greedy_no_draft_logits_reject(self):
        """Non-greedy without draft logits: draft token impossible under target.

        draft_log_prob = 0 and target_log_prob << log(2^-31) for every
        philox draw, so acceptance is deterministic (nothing accepted).
        """
        num_reqs = 1
        num_spec_steps = 3
        num_logits = num_spec_steps + 1
    ...
    def test_non_greedy_varying_temps(self, temp):
        """Non-greedy: near-one-hot target accepts at every temperature.

        draft_log_prob <= 0 and target_log_prob ~= 0 > log(u) + draft_log_prob
        for every philox draw u < 1, independent of the temperature scale.
        """
        num_reqs = 1
        num_spec_steps = 2
        num_logits = num_spec_steps + 1
    ...
    def test_non_greedy_sampled_output(self):
        ...
        # Deterministic accept: near-one-hot target at each draft token.
        for i in range(num_logits - 1):
            target_logits[i, draft_sampled[i + 1]] = 100.0
        temperature = torch.tensor([1.0], dtype=torch.float32, device=self.device)
        seeds = torch.tensor([42], dtype=torch.int64, device=self.device)
    ...
    def test_mixed_greedy_and_non_greedy(self):
        ...
        # Req 1 occupies logits [3, 6). Make its target near-one-hot at the
        # draft tokens so the non-greedy philox draw accepts deterministically.
        for i in range(num_spec_steps):
            logit_idx = 3 + i
            target_logits[logit_idx, draft_sampled[logit_idx + 1]] = 100.0
