...
"""Benchmark postprocess_mamba_fused_kernel on NPU."""
...
def case_postprocess_mamba_fused(device):
    for num_reqs_val, num_layers_val in ((4, 1), (16, 1)):
        ...
        def fn(_grid=grid, _nat=num_accepted_tokens, _msi=mamba_state_idx, _nst=num_scheduled_tokens, _nct=num_computed_tokens, _ndt=num_draft_tokens, _btp=block_table_ptrs, _bts=block_table_stride_req, _sba=state_base_addrs, _sbs=state_block_strides, _ses=state_elem_sizes, _sis=state_inner_sizes, _scw=state_conv_widths, _sgi=state_group_indices, _sdrc=state_dim_row_count, _sdrs=state_dim_row_stride, _nao=num_accepted_tokens_out, _nr=num_reqs_val, _bs=block_size_val):
            postprocess_mamba_fused_kernel[_grid](_nat, _msi, _nst, _nct, _ndt, _btp, _bts, _sba, _sbs, _ses, _sis, _scw, _sgi, _sdrc, _sdrs, _nao, None, _nr, block_size=_bs, COPY_BLOCK_SIZE=1024, CONV_STATE_DIM_FIRST=False, HAS_IDX_MAPPING=False, PRECOMPUTED_NEW_COMPUTED=False)
            holder['out'] = _nao
            return holder['out']
...
def main() -> None:
    ...
    for spec, fn, checksum in case_postprocess_mamba_fused(args.device):
        latency_us, _ = bench_npu(fn, args.warmup, args.repeat)
        print(f"op=postprocess_mamba_fused_kernel {spec} latency_us={latency_us:.2f} checksum={checksum()}")
