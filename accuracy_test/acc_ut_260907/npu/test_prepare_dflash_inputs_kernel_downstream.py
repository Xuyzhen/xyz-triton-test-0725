# GENERATED STRICT UT. Source: accuracy_test/codex/missing_accuracy_tests/test_prepare_dflash_inputs_kernel_ascend_patch.py
...
- Adds context-parallel (DCP) args (vllm-project/vllm#52188): cp_rank scalar
  input plus CP_SIZE / CP_INTERLEAVE constexpr flags. On Ascend CP is not
  wired (CP_SIZE=1, CP_INTERLEAVE=False, cp_rank=0), so the DCP args do not
  change the numerics; they only shift the kernel signature.
...
Kernel signature:
    _prepare_dflash_inputs_kernel_ascend(
        ...
        cp_rank,                        # context-parallel rank (0 when CP off)
        SAMPLE_FROM_ANCHOR: tl.constexpr,   # whether to sample from anchor
        PAD_SLOT_ID: tl.constexpr,      # padding slot ID
        CP_SIZE: tl.constexpr,          # context-parallel size (1 on Ascend)
        CP_INTERLEAVE: tl.constexpr,    # interleave mode flag (False on Ascend)
...
class TestPrepareDFlashInputsKernelAscendPatch:
    ...
    def test_prepare_dflash_inputs(
        ...
        if _DFLASH_KERNEL_HAS_DCP_ARGS:
            # CP is not wired on Ascend: rank 0 of a single-rank group.
