...
Kernel control flow per (logit_idx, block_idx) program:
  ...
  3. non-greedy branch: writes target_local_max and target_local_sumexp.
     ...
     (computed on raw draft_logits WITHOUT temperature scaling).
     target_local_argmax is NOT written in this branch.
        ...
        for li in range(total_num_logits):
            ...
            for bi in range(vocab_num_blocks):
                ...
                if temp == 0.0:
                    ...
                    # draft stats: only when HAS_DRAFT_LOGITS, and computed on
                    # raw draft_logits WITHOUT temperature scaling.
                    # Indexing: draft_logits[rs, draft_step, :]
                    if has_draft_logits:
                        drf_block = drf_cpu[rs, draft_step, start:end]
