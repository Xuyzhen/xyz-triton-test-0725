# GENERATED STRICT UT. Source: accuracy_test/codex/existing_accuracy_tests/from_vllm_ascend/test_bad_words.py
...
# Accuracy UT source: vllm-ascend-xyz/tests/e2e/nightly/single_node/ops/singlecard_ops/triton/test_bad_words.py
# Kernel source: vllm-ascend-xyz/vllm_ascend/worker/v2/sample/bad_words.py
# Coverage: _bad_words_kernel via apply_bad_words

# SPDX-License-Identifier: Apache-2.0
# Test vllm.v1.worker.gpu.sample.bad_words.apply_bad_words (Triton-Ascend).
...
def test_apply_bad_words_different_shapes(
    num_tokens, vocab_size, num_requests, num_bad_words_per_req, bad_word_length, device="cuda"
):
...
@torch.inference_mode()
def test_apply_bad_words_no_bad_words(device="cuda"):
...
@torch.inference_mode()
def test_apply_bad_words_edge_cases(device="cuda"):
...
@torch.inference_mode()
def test_apply_bad_words_token_limit(device="cuda"):
