#!/usr/bin/env bash
#
# 一键运行 acc_ut_260914_f028_main0914 全部 NPU 侧算子精度测试。
#
...
#（先 source CANN 的 set_env.sh）。
#
# 用法：
#   bash run_npu.sh                          # 跑全部 NPU 侧（逐文件隔离，首错即停）
#   bash run_npu.sh npu/test_selector_walk_kernel.py  # 透传给 pytest 的部分运行（完整执行）
#   bash run_npu.sh -k "selector"            # 关键字过滤（完整执行）
#
set -euo pipefail
...
echo "=================================================="
echo "  acc_ut_260914_f028_main0914 NPU 侧算子精度测试（逐文件隔离进程）"
echo "=================================================="
