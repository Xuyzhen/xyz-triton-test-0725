...
# strict_ut_027 shared dual-side UT for _cache_inputs_kernel.
...
# re-exported by gpu/test_cache_inputs_kernel.py and
# npu/test_cache_inputs_kernel.py. The side runtime (CUDA or Ascend NPU) is
# injected through the ``rt`` pytest fixture.
#
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (16, 1024, 3, "full_window", True, 256),
        (16, 2048, 4, "full_window", True, 1024),
        (32, 512, 4, "full_window", False, 128),  # large batch
        (16, 64, 3, "mixed", True, 8),            # large batch, mixed
        (32, 1024, 5, "mixed", True, 512),        # large batch + max steps
        (8, 5120, 3, "mixed", True, 512),         # production hidden size
        (4, 7168, 5, "full_window", True, 1024),  # large hidden + max steps
    ],
    ids=lambda v: str(v),
)
def test_cache_inputs(
    ...
    # query lens go up to cache_window_size + 5 (mixed/full_window)
    num_tokens = max(256, num_reqs * (num_speculative_steps + 8))
